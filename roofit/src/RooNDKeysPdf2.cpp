 ...
 *    File: $Id: RooNDKeysPdf2.cxx 31258 2009-11-17 22:41:06Z wouter $
 * Authors:                                                                  *
...
#include "RooChangeTracker2.h"

#include "TError.h"

using namespace std;

ClassImp(RooNDKeysPdf2);
...
RooNDKeysPdf2::RooNDKeysPdf2(const char *name, const char *title, const RooArgList &varList, const RooDataSet &data,
     ...
     _nSigma(nSigma), _weights(&_weights0), _rotate(rotate), _sortInput(sortInput), _nAdpt(1), _tracker(0)
{
  ...
  for (const auto var : varList) {
    if (!dynamic_cast<const RooAbsReal*>(var)) {
      coutE(InputArguments) << "RooNDKeysPdf2::ctor(" << GetName() << ") ERROR: variable " << var->GetName()
...
RooNDKeysPdf2::RooNDKeysPdf2(const char *name, const char *title, const RooArgList &varList, const TH1 &hist,
                           TString options, Double_t rho, Double_t nSigma, Bool_t rotate, Bool_t sortInput)
   : RooAbsPdf(name, title), _varList("varList", "List of variables", this),
     ...
     _nSigma(nSigma), _weights(&_weights0), _rotate(rotate), _sortInput(sortInput), _nAdpt(1), _tracker(0)
{
  ...
  if( _varList.getSize() != rho.GetNrows() ) {
     coutE(InputArguments)
        << "ERROR:  RooNDKeysPdf2::RooNDKeysPdf2() : The vector-size of rho is different from that of varList."
  ...
  for (Int_t j = 0; j < rho.GetNrows(); j++) {
     _rho[j] = rho[j]; /*cout<<"RooNDKeysPdf2 ctor, _rho["<<j<<"]="<<_rho[j]<<endl;*/
  }
     ...
     _nSigma(nSigma), _weights(&_weights0), _rotate(rotate), _sortInput(sortInput), _nAdpt(1)
{
   ...
   for (unsigned int i=0; i < rhoList.size(); ++i) {
      ...
      if (!dynamic_cast<const RooAbsReal *>(rho)) {
         coutE(InputArguments) << "RooNDKeysPdf2::ctor(" << GetName() << ") ERROR: parameter " << rho->GetName()
   ...
   if ((_varList.getSize() != _rhoList.getSize())) {
      coutE(InputArguments) << "ERROR:  RooNDKeysPdf2::RooNDKeysPdf2() : The size of rhoList is different from varList."
                            << "Unable to create the PDF." << endl;
      assert(_varList.getSize() == _rhoList.getSize());
   }

   // keep track of changes in rho parameters
   _tracker = new RooChangeTracker2("tracker", "track rho parameters", _rhoList, true); // check for value updates
...
RooNDKeysPdf2::RooNDKeysPdf2(const char *name, const char *title, RooAbsReal &x, const RooDataSet &data, Mirror mirror,
     ...
     _nSigma(nSigma), _weights(&_weights0), _rotate(rotate), _sortInput(sortInput), _nAdpt(1), _tracker(0)
{
   ...
   if (mirror != NoMirror) {
      if (mirror != MirrorBoth)
         coutW(InputArguments) << "RooNDKeysPdf2::RooNDKeysPdf2() : Warning : asymmetric mirror(s) no longer supported."
                               << endl;
      _options = "m";
   }
...
RooNDKeysPdf2::RooNDKeysPdf2(const char *name, const char *title, RooAbsReal &x, RooAbsReal &y, const RooDataSet &data,
                           TString options, Double_t rho, Double_t nSigma, Bool_t rotate, Bool_t sortInput)
   : RooAbsPdf(name, title), _varList("varList", "List of variables", this),
     _rhoList("rhoList", "List of rho parameters", this), _data(&data), _options(options), _widthFactor(rho),
     _nSigma(nSigma), _weights(&_weights0), _rotate(rotate), _sortInput(sortInput), _nAdpt(1), _tracker(0)
...
RooNDKeysPdf2::RooNDKeysPdf2(const RooNDKeysPdf2 &other, const char *name)
   : RooAbsPdf(other, name), _varList("varList", this, other._varList), _rhoList("rhoList", this, other._rhoList),
     ...
     _nAdpt(other._nAdpt)
{
   _tracker = (other._tracker != NULL ? new RooChangeTracker2(*other._tracker) : NULL);
   ...
   _rangeBoxInfo2 = other._rangeBoxInfo2;
   _fullBoxInfo2 = other._fullBoxInfo2;
...
RooNDKeysPdf2::~RooNDKeysPdf2()
{
  ...
  // delete all the BoxInfo2s map
  while ( !_rangeBoxInfo2.empty() ) {
    map<pair<string,int>,BoxInfo2*>::iterator iter = _rangeBoxInfo2.begin();
    BoxInfo2* box= (*iter).second;
    _rangeBoxInfo2.erase(iter);
    delete box;
  }
}
...
void RooNDKeysPdf2::createPdf(Bool_t firstCall)
{
  if (firstCall) {
    // set options
    setOptions();
...
void RooNDKeysPdf2::setOptions()
{
  ...
  cxcoutD(InputArguments) << "RooNDKeysPdf2::setOptions()    options = " << _options
  ...
  if (_nSigma<2.0) {
    coutW(InputArguments) << "RooNDKeysPdf2::setOptions() : Warning : nSigma = " << _nSigma << " < 2.0. "
...
void RooNDKeysPdf2::initialize()
{
  ...
  if(_nDim==0) {
    coutE(InputArguments) << "ERROR:  RooNDKeysPdf2::initialize() : The observable list is empty. "
           << "Unable to begin generating the PDF." << endl;
    R__ASSERT (_nDim!=0);
  }

  if(_nEvents==0) {
    coutE(InputArguments) << "ERROR:  RooNDKeysPdf2::initialize() : The input data set is empty. "
           << "Unable to begin generating the PDF." << endl;
  ...
  BoxInfo2Init(&_fullBoxInfo2,0,0xFFFF);
...
void RooNDKeysPdf2::loadDataSet(Bool_t firstCall) const
{
  ...
  coutI(Contents) << "RooNDKeysPdf2::loadDataSet(" << this << ")"
...
void RooNDKeysPdf2::mirrorDataSet() const
{
  for (Int_t j=0; j<_nDim; j++) {
...
void RooNDKeysPdf2::loadWeightSet() const
{
  _wMap.clear();
  ...
  coutI(Contents) << "RooNDKeysPdf2::loadWeightSet(" << this << ") : Number of weighted events : " << _wMap.size() << endl;
}
...
void RooNDKeysPdf2::calculateShell(BoxInfo2* bi) const
{
  for (Int_t j=0; j<_nDim; j++) {
  ...
  coutI(Contents) << "RooNDKeysPdf2::calculateShell() : "
...
void RooNDKeysPdf2::calculatePreNorm(BoxInfo2* bi) const
{
  // box minus shell
  ...
  cxcoutD(Eval) << "RooNDKeysPdf2::calculatePreNorm() : "
...
void RooNDKeysPdf2::sortDataIndices(BoxInfo2* bi)
{
  ...
  for (Int_t j=0; j<_nDim; j++) {
    cxcoutD(Eval) << "RooNDKeysPdf2::sortDataIndices() : Number of sorted events : " << _sortTVIdcs[j].size() << endl;
  }
}

////////////////////////////////////////////////////////////////////////////////

void RooNDKeysPdf2::calculateBandWidth() const
{
  cxcoutD(Eval) << "RooNDKeysPdf2::calculateBandWidth()" << endl;

  // non-adaptive bandwidth
  // (default, and needed to calculate adaptive bandwidth)

  if(!_options.Contains("a")) {
      cxcoutD(Eval) << "RooNDKeysPdf2::calculateBandWidth() Using static bandwidth." << endl;
  }
  ...
  // adaptive width
  if (_options.Contains("a")) {
     cxcoutD(Eval) << "RooNDKeysPdf2::calculateBandWidth() Using adaptive bandwidth." << endl;
...
Double_t RooNDKeysPdf2::gauss(vector<Double_t>& x, vector<vector<Double_t> >& weights) const
{
...
void RooNDKeysPdf2::loopRange(vector<Double_t>& x, map<Int_t,Bool_t>& ibMap) const
{
   ibMap.clear();
   TVectorD xRm(_nDim);
   TVectorD xRp(_nDim);
...
void RooNDKeysPdf2::BoxInfo2Init(BoxInfo2* bi, const char* rangeName, Int_t /*code*/) const
{
...
Double_t RooNDKeysPdf2::evaluate() const
{
...
Int_t RooNDKeysPdf2::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const
{
...
Double_t RooNDKeysPdf2::analyticalIntegral(Int_t code, const char* rangeName) const
{
  cxcoutD(Eval) << "Calling RooNDKeysPdf2::analyticalIntegral(" << GetName() << ") with code " << code
  ...
  if (rangeName) {
    ...
    bi = _rangeBoxInfo2[make_pair(rangeNameStr,code)] ;
    if (!bi) {
      bi = new BoxInfo2 ;
      _rangeBoxInfo2[make_pair(rangeNameStr,code)] = bi ;
      BoxInfo2Init(bi,rangeName,code);
    }
  } else bi= &_fullBoxInfo2 ;
  ...
  // reset
  if (newBounds) {
    cxcoutD(Eval) << "RooNDKeysPdf2::analyticalIntegral() : Found new boundaries ... " << (rangeName?rangeName:"<none>") << endl;
    BoxInfo2Init(bi,rangeName,code);
  }
  ...
  if (!bi->filled || newBounds) {
    ...
    calculatePreNorm(bi);
    bi->filled = kTRUE;
    const_cast<RooNDKeysPdf2*>(this)->sortDataIndices(bi);
  }
  ...
  if (_mirror && bi->netFluxZ) {
    ...
    cxcoutD(Eval) << "RooNDKeysPdf2::analyticalIntegral() : Using mirrored normalization : " << bi->nEventsBW << endl;
    return bi->nEventsBW;
  }
  ...
  else
  {
   ...
   if (chi[j]>0) // inVarRange
    ...
    cxcoutD(Eval) << "RooNDKeysPdf2::analyticalIntegral() : Final normalization : " << norm << " " << bi->nEventsBW << endl;
    return norm;
  }
}
...
RooDataSet* RooNDKeysPdf2::createDatasetFromHist(const RooArgList &varList, const TH1 &hist) const
{
   ...
   for (const auto var : varList) {
      if (!dynamic_cast<RooRealVar *>(var)) {
         coutE(InputArguments) << "RooNDKeysPdf2::createDatasetFromHist(" << GetName() << ") WARNING: variable "
...
TMatrixD RooNDKeysPdf2::getWeights(const int &k) const
{
   TMatrixD mref(_nEvents, _nDim + 1);

   cxcoutD(Eval) << "RooNDKeysPdf2::getWeights() Return evaluated weights." << endl;
   ...
   ////////////////////////////////////////////////////////////////////////////////
void RooNDKeysPdf2::updateRho() const
{
