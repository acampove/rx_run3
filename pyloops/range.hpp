...
namespace iter {
    ...
    template < typename T > constexpr impl::Range< T > range(T) noexcept;
    template < typename T > constexpr impl::Range< T > range(T, T) noexcept;
    template < typename T > constexpr impl::Range< T > range(T, T, T) noexcept;
}   // namespace iter

namespace iter {
    namespace detail {
        template < typename T, bool IsFloat = std::is_floating_point< T >::value > class RangeIterData;

        // everything except floats
        template < typename T > class RangeIterData< T, false > {
          private:
            T value_{};
            T step_{};
          ...
          public:
            ...
            constexpr T step() const noexcept { return this->step_; }

            void inc() noexcept { this->value_ += step_; }

            constexpr bool operator==(const RangeIterData & other) const noexcept { return this->value_ == other.value_; }

            constexpr bool operator!=(const RangeIterData & other) const noexcept { return !(*this == other); }
        };

        // float data
        template < typename T > class RangeIterData< T, true > {
          private:
            T           start_{};
            T           value_{};
            T           step_{};
          ...
          public:
            ...
            constexpr bool operator==(const RangeIterData & other) const noexcept {
                ...
                return (this->value_ < other.value_ ? other.value_ - this->value_ : this->value_ - other.value_) < this->step_;
            }
...
template < typename T > class iter::impl::Range {
    // see stackoverflow.com/questions/32174186 about why only specializations
    // aren't marked as friend
    template < typename U > friend constexpr Range< U > iter::range(U) noexcept;
    template < typename U > friend constexpr Range< U > iter::range(U, U) noexcept;
    template < typename U > friend constexpr Range< U > iter::range(U, U, U) noexcept;
  ...
  private:
    ...
    constexpr Range(T in_stop) noexcept
        : start{0}
        , stop{in_stop}
        , step{1} {}
  ...
  public:
    ...
    class Iterator : public std::iterator< std::forward_iterator_tag, T, std::ptrdiff_t, T *, T > {
      private:
        ...
        static bool not_equal_to_impl(const Iterator & iter, const Iterator & end_iter, std::true_type) noexcept {
            assert(!iter.is_end);
        ...
        static bool not_equal_to_impl(const Iterator & iter, const Iterator & end_iter, std::false_type) noexcept {
            assert(!iter.is_end);
            assert(end_iter.is_end);
            return !(iter.data.step() > 0 && iter.data.value() >= end_iter.data.value()) && !(iter.data.step() < 0 && iter.data.value() <= end_iter.data.value());
        }

        static bool not_equal_to_end(const Iterator & lhs, const Iterator & rhs) noexcept {
            if (rhs.is_end) { return not_equal_to_impl(lhs, rhs, std::is_unsigned< T >{}); }
      ...
      public:
        ...
        constexpr T operator*() const noexcept { return this->data.value(); }
        ...
        bool operator!=(const Iterator & other) const noexcept {
            if (this->is_end && other.is_end) { return false; }

            if (!this->is_end && !other.is_end) { return this->data != other.data; }
    ...
    constexpr Iterator end() const noexcept { return {stop, step, true}; }
};

template < typename T > constexpr iter::impl::Range< T > iter::range(T stop) noexcept { return {stop}; }

template < typename T > constexpr iter::impl::Range< T > iter::range(T start, T stop) noexcept { return {start, stop}; }

template < typename T > constexpr iter::impl::Range< T > iter::range(T start, T stop, T step) noexcept { return step == T(0) ? impl::Range< T >{0} : impl::Range< T >{start, stop, step}; }
