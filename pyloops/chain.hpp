...
namespace iter {
    namespace impl {
        template < typename Container, typename... RestContainers > class Chained;

        template < typename Container > class Chained< Container >;

        template < typename Container > class ChainedFromIterable;
...
template < typename Container, typename... RestContainers > class iter::impl::Chained {
    static_assert(are_same< iterator_deref< Container >, iterator_deref< RestContainers >... >::value, "All chained iterables must have iterators that "
                                                                                                       "dereference to the same type, including cv-qualifiers "
                                                                                                       "and references.");

    friend class ChainMaker;
    template < typename, typename... > friend class Chained;
  ...
  public:
    ...
    class Iterator : public std::iterator< std::input_iterator_tag, iterator_traits_deref< Container > > {
      ...
      public:
        Iterator(iterator_type< Container > && s_begin, iterator_type< Container > && s_end, RestIter && in_rest_iter)
        ...
        Iterator & operator++() {
            if (this->at_end) {
                ...
                ++this->sub_iter;
                if (!(this->sub_iter != this->sub_end)) { this->at_end = true; }
            }
            return *this;
        }
        ...
        bool operator!=(const Iterator & other) const { return this->sub_iter != other.sub_iter || this->rest_iter != other.rest_iter; }

        bool operator==(const Iterator & other) const { return !(*this != other); }

        iterator_deref< Container > operator*() { return this->at_end ? *this->rest_iter : *this->sub_iter; }

        iterator_arrow< Container > operator->() { return this->at_end ? apply_arrow(this->rest_iter) : apply_arrow(this->sub_iter); }
    };

    Iterator begin() { return {std::begin(this->container), std::end(this->container), std::begin(this->rest_chained)}; }

    Iterator end() { return {std::end(this->container), std::end(this->container), std::end(this->rest_chained)}; }
};
template < typename Container > class iter::impl::Chained< Container > {
    friend class ChainMaker;
    template < typename, typename... > friend class Chained;
  ...
  public:
    ...
    class Iterator : public std::iterator< std::input_iterator_tag, iterator_traits_deref< Container > > {
      ...
      public:
        Iterator(const iterator_type< Container > & s_begin, const iterator_type< Container > & s_end)
            : sub_iter{s_begin}
            , sub_end{s_end} {}
...
template < typename Container > class iter::impl::ChainedFromIterable {
  private:
    Container container;
    friend class ChainMaker;
  ...
  public:
    ...
    class Iterator : public std::iterator< std::input_iterator_tag, iterator_traits_deref< iterator_deref< Container > > > {
      private:
        ...
        static std::unique_ptr< SubIter > clone_sub_pointer(const SubIter * sub_iter) { return std::unique_ptr< SubIter >{sub_iter ? new SubIter{*sub_iter} : nullptr}; }

        bool sub_iters_differ(const Iterator & other) const {
            if (this->sub_iter_p == other.sub_iter_p) { return false; }
      ...
      public:
        Iterator(iterator_type< Container > && top_iter, iterator_type< Container > && top_end)
        ...
        bool operator!=(const Iterator & other) const { return this->top_level_iter != other.top_level_iter || this->sub_iters_differ(other); }

        bool operator==(const Iterator & other) const { return !(*this != other); }

        iterator_deref< iterator_deref< Container > > operator*() { return **this->sub_iter_p; }

        iterator_arrow< iterator_deref< Container > > operator->() { return apply_arrow(*this->sub_iter_p); }
    };
...
class iter::impl::ChainMaker {
  public:
    ...
    template < typename... Containers > Chained< Containers... > operator()(Containers &&... cs) const { return {std::forward< Containers >(cs)...}; }

    // chain.from_iterable()
    template < typename Container > ChainedFromIterable< Container > from_iterable(Container && container) const { return {std::forward< Container >(container)}; }
};
