...
def get_ratio(kind, brem, trig, dset):
    mc_path  = f'output/resolution/mc/json/{data.version}/{trig}_{dset}/{kind}_brem_{brem}.json'
    dt_path  = f'output/resolution/data/json/{data.version}/{trig}_{dset}/{kind}_brem_{brem}.json'
...
def main():
    ...
    for trig in data.l_trig:
        for dset in data.l_dset:
            ofile = ROOT.TFile(f'{data.out_dir}/{dset}_{trig}.root', 'recreate')
