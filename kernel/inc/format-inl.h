...
namespace internal {
    ...
    template < typename Char > FMT_FUNC Char thousands_sep_impl(locale_ref loc) { return std::use_facet< std::numpunct< Char > >(loc.get< std::locale >()).thousands_sep(); }
}   // namespace internal
#else
...
namespace internal {
    template < typename T > int char_traits< char >::format_float(char * buf, std::size_t size, const char * format, int precision, T value) { return precision < 0 ? FMT_SNPRINTF(buf, size, format, value) : FMT_SNPRINTF(buf, size, format, precision, value); }

    template < typename T > int char_traits< wchar_t >::format_float(wchar_t * buf, std::size_t size, const wchar_t * format, int precision, T value) { return precision < 0 ? FMT_SWPRINTF(buf, size, format, value) : FMT_SWPRINTF(buf, size, format, precision, value); }
...
#define FMT_POWERS_OF_10(factor) factor * 10, factor * 100, factor * 1000, factor * 10000, factor * 100000, factor * 1000000, factor * 10000000, factor * 100000000, factor * 1000000000
    ...
    FMT_FUNC bool grisu2_gen_digits(char * buf, int & size, uint32_t hi, uint64_t lo, int & exp, uint64_t delta, const fp & one, const fp & diff, int max_digits) {
        ...
        while (exp > 0) {
            ...
            if (remainder <= delta || size > max_digits) { return grisu2_round(buf, size, max_digits, delta, remainder, static_cast< uint64_t >(data::POWERS_OF_10_32[exp]) << -one.e, diff.f, exp); }
        }
...
#define FMT_FALLTHROUGH
    ...
    template < typename Handler > FMT_FUNC void grisu2_prettify(const gen_digits_params & params, int size, int exp, Handler && handler) {
        if (!params.fixed) {
    ...
    template < typename Double > FMT_FUNC typename std::enable_if< sizeof(Double) == sizeof(uint64_t), bool >::type grisu2_format(Double value, buffer & buf, core_format_specs specs) {
    ...
    template < typename Double > void sprintf_format(Double value, internal::buffer & buf, core_format_specs spec) {
...
FMT_FUNC void internal::format_windows_error(internal::buffer & out, int error_code, string_view message) FMT_NOEXCEPT {
    FMT_TRY {
        ...
        for (;;) {
            ...
            int       result         = FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, FMT_NULL, error_code, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), system_message, static_cast< uint32_t >(buf.size()), FMT_NULL);
            if (result != 0) {
...
FMT_FUNC void report_system_error(int error_code, fmt::string_view message) FMT_NOEXCEPT { report_error(format_system_error, error_code, message); }

#if FMT_USE_WINDOWS_H
FMT_FUNC void report_windows_error(int error_code, fmt::string_view message) FMT_NOEXCEPT { report_error(internal::format_windows_error, error_code, message); }
#endif
