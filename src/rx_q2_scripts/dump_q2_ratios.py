...
#----------------------------
def par_to_res(par, d_par):
    name  = {'rsg' : 'sg', 'dmu' : 'mu', 'mMC' : 'mu', 'mDT' : 'mu'}[par]
    d_res = {}
...
def divide(num, den):
    ...
    if bad_num and bad_den:
        ...
        raise ValueError
#-------------------------------------
def calculate_par(par, d_dt_str, d_mc_str):
    d_rat_str = {}
    for key in d_dt_str.keys():
        ...
        if   par == 'mMC':
            d_rat_str[key] = val_mc
        elif par == 'mDT':
            d_rat_str[key] = val_dt
        elif par == 'dmu':
    ...
    return d_rat_str
#-------------------------------------
def get_par(par, brem, trig, dset):
    mc_path  = f'{data.inp_dir}/get_q2_resolutions/mc/json/{data.version}/{trig}_{dset}/par_brem_{brem}.json'
    dt_path  = f'{data.inp_dir}/get_q2_resolutions/data/json/{data.version}/{trig}_{dset}/par_brem_{brem}.json'

    d_mc_str = utnr.load_json(mc_path)
    d_dt_str = utnr.load_json(dt_path)

    d_mc_str = par_to_res(par, d_mc_str)
    d_dt_str = par_to_res(par, d_dt_str)

    d_par_str= calculate_par(par, d_dt_str, d_mc_str)
        ...
        for dset in data.l_dset:
            ...
            for par in data.l_par:
                for brem in data.l_brem:
                    d_rat_str = get_par(par, brem, trig, dset)
                    par_path = f'{data.out_dir}/{trig}_{dset}/{par}_brem_{brem}.json'
                    data.log.visible(f'Saving to: {par_path}')
                    utnr.dump_json(d_rat_str, par_path)

                    h_rat = convert_to_hist(d_rat_str)
                    h_rat.SetName(f'h_{par}_brem_{brem}')
                    h_rat.Write()
            ofile.Close()
#-------------------------------------
