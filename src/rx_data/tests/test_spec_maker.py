...
def test_patching(sample : str, trigger : Trigger, tmp_path : Path) -> None:
    '''
...
def test_inclusive(sample : str, trigger : Trigger, tmp_path : Path) -> None:
    '''
    Disable patching
    '''
    with SpecMaker.cache_directory(path = tmp_path):
...
# ------------------------------------------------------
@pytest.mark.parametrize('sample, trigger', _PATCHING_SAMPLES)
def test_skip_patching(sample : str, trigger : Trigger, tmp_path : Path) -> None:
    '''
    Disable patching
    '''
    with SpecMaker.cache_directory(path = tmp_path):
...
def test_combined(sample : str, trigger : Trigger, tmp_path : Path) -> None:
    '''
    Return path to specification of combined sample
    '''
    with SpecMaker.cache_directory(path = tmp_path):
...
# ------------------------------------------------------
@pytest.mark.parametrize('sample, trigger', _GOODSAMPLES)
def test_per_file(sample : str, trigger : Trigger, tmp_path : Path) -> None:
    '''
    Return path to specification of combined sample
    '''
    with SpecMaker.cache_directory(path = tmp_path):
...
def test_exclude_friends(friend : str, sample : str, trigger : Trigger, tmp_path : Path):
    '''
...
@pytest.mark.parametrize('sample' , ['DATA_24_MagDown_24c2'])
@pytest.mark.parametrize('trigger', [Trigger.rk_ee_os, Trigger.rk_mm_os])
def test_only_friends(sample : str, trigger : Trigger, tmp_path : Path):
    '''
    Tests getting data with a custom version for a given tree, either friend or main
    '''
    s_friend = {'mva', 'hop'}
...
@pytest.mark.parametrize('sample' , ['DATA_24_MagDown_24c2'])
@pytest.mark.parametrize('trigger', [Trigger.rk_ee_os, Trigger.rk_mm_os])
def test_project(sample : str, trigger : Trigger, project : Project, tmp_path : Path):
    '''
    Tests getting data with a custom version for a given tree, either friend or main
    '''
...
def test_nopid(sample : str, tmp_path : Path):
    '''
