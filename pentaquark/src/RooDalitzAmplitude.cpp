...
Double_t C2[6][2] = {1.0000000000000000e+00, -5.7735026918962584e-01, -5.7735026918962573e-01, 4.4721359549995765e-01, 4.4721359549995754e-01, -3.7796447300922748e-01, -3.7796447300922759e-01, 3.3333333333333659e-01, 3.3333333333332898e-01, -3.0151134457775197e-01, -3.0151134457772666e-01, 2.7735009811269257e-01};
...
Double_t _FFR(3.0);
Double_t _FFB(3.0);
RooDalitzAmplitude::RooDalitzAmplitude(const char * name, const char * title, RooArgList & values, TList * resList, TList * resListZ, const TString mcdata, RooDataSet & data, bool isPP, bool isCach)
...
// MCDATA(mcdata)
{
    ...
    for (int imc = 0; imc < smcnum; imc++) {
        ...
        filldlz(xdlz, mkp, costheta, costheta1, costheta2, phi1, phi2, costhetaB, costhetaZ, costhetaPsi, phiZ, phiPsi, phiMu, costhetap, alphaMu, mjpsip, sw);
        sumW += sw;
        pdlz->push_back(xdlz);
    }
    ...
    if (_isCach) {
        for (int i = 0; i < smcnum; ++i) {
            ...
            filldlz(xdlz, mkp, costheta, costheta1, costheta2, phi1, phi2, costhetaB, costhetaZ, costhetaPsi, phiZ, phiPsi, phiMu, costhetap, alphaMu, mjpsip);
            PDLZ->push_back(xdlz);
        }
    } else {
        PDLZ->reserve(0);
...
Double_t RooDalitzAmplitude::evaluate(Double_t * Values) {
    ...
    if (!kine_limitsZ(mjpsip)) {
        std::cout << "why mjpsip " << mjpsip << std::endl;
        return 0.0;
    }
    //   double phi =  ((RooAbsReal&)Values[6];
    filldlz(*xdlz1, mkp, costheta, costheta1, costheta2, phi1, phi2, costhetaB, costhetaZ, costhetaPsi, phiZ, phiPsi, phiMu, costhetap, alphaMu, mjpsip);
...
void RooDalitzAmplitude::genToy(const char * name, bool verbose) const {
    ...
    if (verbose) {
        for (int ii = 0; ii < NR; ++ii) tree->Branch(TString("w") + TString((_resList->At(ii))->GetName()), &wl[ii + 1], TString("w") + TString((_resList->At(ii))->GetName()) + TString("/D"));
        for (int ii = 0; ii < NZ; ++ii) tree->Branch(TString("w") + TString((_resListZ->At(ii))->GetName()), &wz[ii + 1], TString("w") + TString((_resListZ->At(ii))->GetName()) + TString("/D"));
    }
    ...
    for (int i = 0; i < smcnum; i++) {
        ...
        if (verbose) {
            for (int ii = 0; ii < NR; ++ii) wl[ii + 1] = GetAmp2sForInt(p, ii) * sweight;
            for (int ii = 0; ii < NZ; ++ii) wz[ii + 1] = GetAmp2sForInt(p, ii + NR) * sweight;
        }
        tree->Fill();
    }
    tree->Write();
...
Int_t RooDalitzAmplitude::getAnalyticalIntegral(RooArgSet & allVars, RooArgSet & analVars, const char * /*rangeName*/) const {
    ...
    List.add();
    */
    if (matchArgs(allVars, analVars, _values)) return 7;
    return 0;
}
                ...
                for (int ir1 = 0; ir1 < NR; ++ir1) {
                    for (int ir2 = 0; ir2 < NR; ++ir2) {
                        for (int m1 = 0; m1 < 6; ++m1) {
                            for (int m2 = 0; m2 < 6; ++m2) { AA[i][j][k][ir1][ir2][m1][m2] = TComplex(0., 0.); }
                        }
                    }
                }
            }
                ...
                for (int ir1 = 0; ir1 < NZ; ++ir1) {
                    for (int ir2 = 0; ir2 < NZ; ++ir2) {
                        for (int m1 = 0; m1 < 12; ++m1) {
                            for (int m2 = 0; m2 < 12; ++m2) { AAZ[i][j][k][ir1][ir2][m1][m2] = TComplex(0., 0.); }
                        }
                    }
                }
            }
        ...
        for (int j = 0; j < 2; ++j) {
            for (int k = 0; k < 2; ++k) {
                for (int ir1 = 0; ir1 < NZ; ++ir1) {
                    for (int ir2 = 0; ir2 < NR; ++ir2) {
                        for (int m1 = 0; m1 < 12; ++m1) {
                            for (int m2 = 0; m2 < 6; ++m2) { ABZ[i][j][k][ir1][ir2][m1][m2] = TComplex(0., 0.); }
                        }
                    }
                }
            }
...
Double_t RooDalitzAmplitude::analyticalIntegral(Int_t code, const char * rangeName) const {
    ...
    while (RooArgList * Ires = (RooArgList *) coefIter->Next()) {
        ...
        if ((fabs(_resParCache[2 * ir + 0] - m0) > 1e-20) || (fabs(_resParCache[2 * ir + 1] - g0) > 1e-20)) { ResetDef(ir); }
...
TComplex RooDalitzAmplitude::MEang(double hel0, double hel3, int delhelmu, double J, double hel1, int hel2, Jpsihh_dlz & dlz) const {
    ...
    return TComplex(1, partphi, 1) * dfunhalf(theta, (int) (hel0 * 2.), (int) ((hel1 - hel2) * 2.)) * dfun(theta1, J, hel1, hel3) * dfunone(theta2, hel2, delhelmu);
...
TComplex RooDalitzAmplitude::MEangZ(double helB, double hel3, int delhelmu, double JZ, double helZ, int helpsi, double help, Jpsihh_dlz & dlz) const {
    ...
    return TComplex(1, partphi, 1) * dfunone(thetaPsi, helpsi, delhelmu) * dfunhalf(thetaB, (int) (helB * 2.), (int) (helZ * 2.)) * dfunhalf(thetap, (int) (help * 2.), (int) (hel3 * 2.)) *   // hel3 = hel_p in /\*
        ...
        for (int i = 0; i <= 1; ++i) {   // delmu
            ...
            for (int j = 0; j <= 1; ++j) {   // hel0
                ...
                for (int k = 0; k <= 1; ++k) {   // hel3 proton
                    ...
                    for (int m = 0; m < 6; ++m) {
                        //	    amp[ir][i][j][k][m] = 0.0;
                        if (fabs(S - 0.5) < 1e-10 && m >= 4) { continue; }
...
Double_t RooDalitzAmplitude::GetAmp2s(Jpsihh_dlz & dlz, int isintcal) const {
    ...
    for (int i = 0; i <= 1; ++i) {
        for (int j = 0; j <= 1; ++j) {
            for (int k = 0; k <= 1; ++k) { H[i][j][k] = TComplex(0, 0); }
        }
    }
    ...
    while (RooArgList * Ires = (RooArgList *) coefIter->Next()) {
        ...
        if (notdefined
            ...
            || (fabs(_resParCache[2 * ir + 0] - m0) > 1e-20) || (fabs(_resParCache[2 * ir + 1] - g0) > 1e-20)) {
                        ...
                        for (int helpsi = -1; helpsi <= 1; ++helpsi) {
                            for (int ihz = 0; ihz <= 1; ++ihz) {
                                ...
                                for (int ihp = 0; ihp <= 1; ++ihp) {
                                    ...
                                    if ((ampCoff.Rho2()) < 1e-10 && isintcal == 0) {
                                        m++;
                                        continue;
                                    }
    ...
    double totalA = H[0][0][0].Rho2() + H[0][0][1].Rho2() + H[0][1][0].Rho2() + H[0][1][1].Rho2() + H[1][0][0].Rho2() + H[1][0][1].Rho2() + H[1][1][0].Rho2() + H[1][1][1].Rho2();
...
void RooDalitzAmplitude::getIFandFF() const {
    ...
    for (int ir1 = 0; ir1 < NR; ++ir1) {
        for (int ir2 = 0; ir2 < NR; ++ir2) { InteL[ir1][ir2] = 0.0; }
    }
    for (int ir1 = 0; ir1 < NZ; ++ir1) {
        for (int ir2 = 0; ir2 < NR; ++ir2) { InteLZ[ir1][ir2] = 0.0; }
    }
    for (int ir1 = 0; ir1 < NZ; ++ir1) {
        for (int ir2 = 0; ir2 < NZ; ++ir2) { InteZ[ir1][ir2] = 0.0; }
    }
    ...
    for (int ir1 = 0; ir1 < NR; ++ir1) {
        for (int ir2 = 0; ir2 < NR; ++ir2) { IFFF(ir1, ir2) = 100. * InteL[ir1][ir2] / Inte; }
    }
    for (int ir1 = 0; ir1 < NZ; ++ir1) {
        for (int ir2 = 0; ir2 < NZ; ++ir2) { IFFF(ir1 + NR, ir2 + NR) = 100. * InteZ[ir1][ir2] / Inte; }
    }
    for (int ir1 = 0; ir1 < NZ; ++ir1) {
        for (int ir2 = 0; ir2 < NR; ++ir2) {
...
TComplex RooDalitzAmplitude::HelicityCoff(RooArgList ires, Int_t ihel) const { return TComplex(((RooAbsReal &) ires[ihel * 2]).getVal(), ((RooAbsReal &) ires[ihel * 2 + 1]).getVal()); }
...
Double_t RooDalitzAmplitude::F_BW(double r, int L, double p, double p0) const {
    ...
    if (L == 4) return sqrt((pow(z0, 4) + 10 * pow(z0, 3) + 135 * z0 * z0 + 1575 * z0 + 11025) / (pow(z, 4) + 10 * pow(z, 3) + 135 * z * z + 1575 * z + 11025));
    if (L == 5) return sqrt((pow(z0, 5) + 15 * pow(z0, 4) + 315 * pow(z0, 3) + 6300 * pow(z0, 2) + 99225 * z0 + 893025) / (pow(z, 5) + 15 * pow(z, 4) + 315 * pow(z, 3) + 6300 * pow(z, 2) + 99225 * z + 893025));

    return 1.0;
}   //////////////////////////////////////////////////
// Breit Wigner Amplitude
/////////////////////////////////////////////////
bool RooDalitzAmplitude::BW_AMP(double m_r, double gamma_r, double mpp, int LR, double & ampl_r, double & ampl_i) const {
    ampl_r      = 0;
    ampl_i      = 0;
    double m2pp = mpp * mpp;
    //  if(J>2) return false;
...
bool RooDalitzAmplitude::BW_AMPZ(double m_r, double gamma_r, double mpp, int LR, double & ampl_r, double & ampl_i) const {
    ampl_r      = 0;
    ampl_i      = 0;
    double m2pp = mpp * mpp;
    //  if(J>2) return false;
...
void RooDalitzAmplitude::filldlz(Jpsihh_dlz & dlz, Double_t xmkp, Double_t xcostheta, Double_t xcostheta1, Double_t xcostheta2, Double_t xphi1, Double_t xphi2, Double_t xcosthetaB, Double_t xcosthetaZ, Double_t xcosthetaPsi, Double_t xphiZ, Double_t xphiPsi, Double_t xphiMu, Double_t xcosthetap, Double_t xalphaMu, Double_t xmjpsip, Double_t sw) const {
    dlz.mkp    = xmkp;
    dlz.mjpsip = xmjpsip;
...
RooAbsReal * RooDalitzAmplitude::createSUM(RooAbsData & data, const RooCmdArg & arg1, const RooCmdArg & arg2, const RooCmdArg & arg3, const RooCmdArg & arg4, const RooCmdArg & arg5, const RooCmdArg & arg6, const RooCmdArg & arg7, const RooCmdArg & arg8) {
...
RooAbsReal * RooDalitzAmplitude::createSUM(RooAbsData & data, const RooLinkedList & cmdList) {
    ...
    if (cloneData == 2) { cloneData = optConst; }
    ...
    if (ext == 2) {
        ...
        if (ext) { coutI(Minimization) << "p.d.f. provides expected number of events, including extended term in likelihood." << std::endl; }
    }
    ...
    if (extCons) { allConstraints.add(*extCons); }
    ...
    /*  // Include constraints, if any, in likelihood
      ...
      */
    if (optConst) { nll->constOptimizeTestStatistic(RooAbsArg::Activate, optConst > 1); }

    if (doStripDisconnected) { delete cPars; }
